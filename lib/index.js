...
function isTaskResponse(value) {
	...
	return typeof value.id === "string" && typeof value.status === "string" && !Array.isArray(value.data);
}
/** The task id of an async task response. */
function taskIdOf(value) {
	if (typeof value.id !== "string" || value.id === "") throw new ImageApiError("Provider returned an async task without an id.", { retryable: false });
	return value.id;
}
...
var ImageClient = class {
	...
	async pollTask(taskId, fallbackFormat, signal, onProgress, maximumJsonBytes) {
		...
		while (true) {
			...
			let response;
			try {
				response = await this.fetchImpl(statusUrl, {
					method: "GET",
					redirect: "error",
					headers,
					signal
				...
				if (signal.aborted) throw signal.reason;
				throw new ImageApiError("Image generation task status check failed.", {
					retryable: true,
					cause: error
				});
			}
			...
			if (!isRecord$1(value) || typeof value.status !== "string") throw new ImageApiError("Provider returned malformed task status.", { retryable: true });
			...
			if (status === "failed" || status === "error" || status === "cancelled") throw new ImageApiError(`Image generation task ${status}: ${isRecord$1(value.error) && typeof value.error.message === "string" ? value.error.message : typeof value.message === "string" ? value.message : status}`, { retryable: false });
			await wait(TASK_POLL_MS, signal);
		}
	}
	/** Extract images from a completed task's `result.data[]` (url or b64). */
	async imagesFromTaskResult(value, fallbackFormat, signal) {
		const resultValue = value.result;
		const entries = isRecord$1(resultValue) && Array.isArray(resultValue.data) ? resultValue.data : [];
		const images = [];
...
function apply(ctx, config = {}) {
	...
	ctx.effect(() => ctx.connection.rpc.handle(IMAGEN_RPC_CHANNEL, async (endpoint, payload, signal) => {
		...
		if (endpoint === IMAGEN_RPC_ENDPOINT.settingsGet) try {
			return {
				ok: true,
				value: await settingsView(ctx, spec())
			};
		} catch (error) {
			return rpcError(error instanceof Error ? error.message : "Failed to read settings.");
		}
		if (endpoint === IMAGEN_RPC_ENDPOINT.settingsSet) {
			const draft = isRecord(payload.config) ? payload.config : void 0;
			if (draft === void 0) return rpcError("A valid config object is required.");
			try {
				const resolved = resolveConfig(draft);
				const keys = isRecord(payload.keys) ? payload.keys : void 0;
				if (keys !== void 0) for (const [name$1, value] of Object.entries(keys)) {
					if (name$1.trim() === "" || typeof value !== "string" || value.trim() === "") continue;
					const credentials = ctx.get("credentials");
					if (credentials === void 0) throw new Error("The credentials service is not mounted.");
					await credentials.set(credentialRef(name$1.trim()), value);
				}
				const settings = ctx.get("settings");
				if (settings === void 0) throw new Error("The settings service is not mounted; edit ~/.dsh/cordis.patch.yml instead.");
...
async function settingsView(ctx, config) {
	const credentials = ctx.get("credentials");
	const sources = {};
	for (const [name$1, source] of Object.entries(config.sources)) {
	...
	return {
		sources,
		...config.defaultSource === void 0 ? {} : { defaultSource: config.defaultSource },
		save: config.save,
...
function plainConfig(config) {
	const sources = {};
	for (const [name$1, source] of Object.entries(config.sources)) sources[name$1] = {
		baseUrl: source.baseUrl,
		credential: String(source.credential),
		...source.model === void 0 ? {} : { model: source.model }
	};
