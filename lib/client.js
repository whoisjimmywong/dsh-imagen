...
function ImagenSettingsCard({ scope, t }) {
	...
	const [sourceRows, setSourceRows] = (0, react.useState)(() => toSources(value));
	const [defaultSource, setDefaultSource] = (0, react.useState)(value?.defaultSource ?? "");
	const [saveEnabled, setSaveEnabled] = (0, react.useState)(value?.save?.enabled ?? true);
	const [saveDir, setSaveDir] = (0, react.useState)(value?.save?.dir ?? "generated-images");
	const [nameTemplate, setNameTemplate] = (0, react.useState)(value?.save?.nameTemplate ?? "{prompt}-{timestamp}");
	const [discoveryEnabled, setDiscoveryEnabled] = (0, react.useState)(value?.discovery?.enabled ?? true);
	const [extraPatterns, setExtraPatterns] = (0, react.useState)(() => patternsText(value));
	const [defaultSize, setDefaultSize] = (0, react.useState)(value?.defaults?.size ?? "");
	const [defaultQuality, setDefaultQuality] = (0, react.useState)(value?.defaults?.quality ?? "");
	const [defaultFormat, setDefaultFormat] = (0, react.useState)(value?.defaults?.outputFormat ?? "png");
	const [defaultN, setDefaultN] = (0, react.useState)(value?.defaults?.n ?? 1);
	const [timeoutMs, setTimeoutMs] = (0, react.useState)(value?.limits?.timeoutMs ?? 12e4);
	const [maxRetries, setMaxRetries] = (0, react.useState)(value?.limits?.maxRetries ?? 2);
	const [retryBaseMs, setRetryBaseMs] = (0, react.useState)(value?.limits?.retryBaseMs ?? 1e3);
	const [maxConcurrent, setMaxConcurrent] = (0, react.useState)(value?.limits?.maxConcurrent ?? 2);
	const [maxImageBytes, setMaxImageBytes] = (0, react.useState)(value?.limits?.maxImageBytes ?? 2e7);
	const [maxReferenceBytes, setMaxReferenceBytes] = (0, react.useState)(value?.limits?.maxReferenceBytes ?? 1e7);
	...
	(0, react.useEffect)(() => {
		...
		setTimeoutMs(snapshot.value.limits?.timeoutMs ?? 12e4);
		setMaxRetries(snapshot.value.limits?.maxRetries ?? 2);
		setRetryBaseMs(snapshot.value.limits?.retryBaseMs ?? 1e3);
		setMaxConcurrent(snapshot.value.limits?.maxConcurrent ?? 2);
		setMaxImageBytes(snapshot.value.limits?.maxImageBytes ?? 2e7);
		setMaxReferenceBytes(snapshot.value.limits?.maxReferenceBytes ?? 1e7);
		setMessage(void 0);
	}, [snapshot]);
	const dirty = (0, react.useMemo)(() => JSON.stringify(buildDraft()) !== JSON.stringify(snapshot.value ?? {}), [
		sourceRows,
		defaultSource,
		saveEnabled,
		saveDir,
	...
	if (snapshot.status === "unavailable") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: "dshImagenSet__page",
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: t("settingsUnavailable") })
	});
	if (snapshot.status === "loading" || snapshot.value === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: "dshImagenSet__page",
...
function installPluginCard(ctx, t) {
	const webUi = ctx.get("webUiSettings");
	const official = ctx.get("settingsScope");
	const binder = webUi ?? official;
	if (binder === void 0) return;
	const scope = binder.bind({ namespace: "imagen" });
